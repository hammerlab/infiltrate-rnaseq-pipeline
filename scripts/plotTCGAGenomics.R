...
untar <- subset(all_times,Task == 'untar')$Time
kallisto <- subset(all_times,Task == 'kallisto')$Time

all_times <- data.frame(samples,start,untar,trimming,kallisto)
...
all_times$untar <- sapply(all_times$untar,FUN=time_since_global_beginning)
...
untar <- apply(all_times, FUN=time_per_task, MARGIN = 1, column1 = "untar", column2 = "start")
trimming <- apply(all_times, FUN=time_per_task, MARGIN = 1, column1 = "trimming", column2 = "untar")
kallisto <- apply(all_times, FUN=time_per_task, MARGIN = 1, column1 = "kallisto", column2 = "trimming")

percents <- data.frame(samples, trimming, untar, kallisto)

percents <- data.frame(task = c('Untar','Trimming','Kallisto'),
                       value = c(mean(percents$untar) * 100,
...
ggplot(data=disk, aes(x=Time, y=Value, group=Metric, color=Metric, linetype = Metric)) +
  annotate("rect", xmin=0, xmax=percents$bars[1], ymin=0, ymax=300, fill="red", alpha=0.1) +
  annotate("rect", xmin=percents$bars[1], xmax=percents$bars[2], ymin=0, ymax=300, fill="blue", alpha=0.08) +
  annotate("rect", xmin=percents$bars[2], xmax=percents$bars[3], ymin=0, ymax=300, fill="green", alpha=0.08) +
  geom_line(size=1.2) + sar_theme(width = unit(20, "lines")) +
  labs(color = "") +
  scale_colour_brewer(type = "qual", palette = "Dark2", direction = -1) +
  scale_x_continuous(name = "Progress",limits = c(0,102), expand = c(0,0), breaks = seq(from = 0, to = 100, by = 10), labels = sapply(seq(from = 0, to = 100, by = 10), function(x) paste(x,'%', sep = ""))) +
  scale_y_continuous(name = "Transfer Speed (MB/s)",limits = c(-0.5,300), expand = c(0,0), breaks = seq(from = 20, to = 300, by = 20)) +
...
time.untar = ((time.run_docker / 100) * (percents[1,2] / 100)) * 100
time.trimming = ((time.run_docker / 100) * (percents[2,2] / 100)) * 100
time.kallisto = ((time.run_docker / 100) * (percents[3,2] / 100)) * 100

timeline <- data.frame(
  task=as.factor(c("Spin Up","Pull Image","Localize Files","Untaring","Trimming","Kallisto","Delocalize Files")),
  value=c(time.spin_up, time.pull_image, time.localize_files, time.untar, time.trimming, time.kallisto, time.delocalize_files))

timeline$task <- factor(timeline$task,levels(timeline$task))

factors <- c("Spin Up","Trimming","Pull Image","Kallisto","Localize Files","Delocalize Files","Untaring")
...
ggplot(timeline, aes(x=0,y=value,fill=task, width=10)) +
  annotate("text", x=-9, y=timeline$position, label=paste(round(timeline$value,digits = 1),'%',sep = ""), size=10) +
  ...
  scale_x_continuous(limits = c(-9.75,25), expand = c(0,0))
